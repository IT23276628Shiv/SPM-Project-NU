...
export default function RegisterScreen({ navigation }) {
  ...
  const handleRegister = () => {
    ...
    createUserWithEmailAndPassword(authfirebase, email, password)
      .then((userCredential) => {
        const createuserdata = userCredential.user;
        console.log("Created User in firebase : ", createuserdata);
        navigation.navigate("Login");
      })
      .catch((error) => {
        console.log(error.code)
        if (error.code === "auth/email-already-in-use") {
           Alert.alert("Email Already Registered",`"${email}" is already registered. Try login or use another email.`)
        }
