...
router.post('/conversations/start', authMiddleware, async (req, res) => {
  try {
    ...
    // ✅ FIX: Always sort participants to ensure consistent order
    const participantIds = [req.userId.toString(), receiverId.toString()].sort();

    console.log('🔍 Sorted participants:', participantIds);

    // ✅ FIX: Search by both sorted participants
    let conversation = await Conversation.findOne({
      'participants.0': participantIds[0],
      'participants.1': participantIds[1]
    });
    ...
    conversation = await Conversation.create({
      participants: participantIds, // Already sorted
